...
Here, we discuss the ways uncertainties can be applied to surface UTEM data, and how it might impact the inversion result.
...
.. important:: If uncertainties are computed using total field data, we would have very large uncertainties proximal to transmitter loops and we would underfit in this region. If the primary field has not been accounted for exactly, there will be erroneous signatures and proximal to the transmitter loops which are likely to result in the recovery of erroneous structures.
